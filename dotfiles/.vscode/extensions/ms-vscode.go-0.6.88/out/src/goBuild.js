...
function goBuild(fileUri, goConfig, buildWorkspace) {
    ...
    if (buildWorkspace && currentWorkspace && !isTestFile) {
        return goPackages_1.getNonVendorPackages(currentWorkspace).then(pkgs => {
            ...
            buildPromises = pkgs.map(pkgPath => {
                running = true;
                return util_1.runTool(buildArgs.concat(pkgPath), currentWorkspace, 'error', true, null, buildEnv, true, tokenSource.token);
            });
    ...
    running = true;
    return util_1.runTool(buildArgs.concat(importPath), cwd, 'error', true, null, buildEnv, true, tokenSource.token).then(v => {
        updateRunning();
        return v;
    });
}
