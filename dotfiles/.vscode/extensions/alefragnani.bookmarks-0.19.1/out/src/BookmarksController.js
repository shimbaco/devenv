...
class BookmarksController {
    ...
    nextDocumentWithBookmarks(active, direction = BookmarkFile_1.JUMP_FORWARD) {
        ...
        return new Promise((resolve, reject) => {
            if (direction === BookmarkFile_1.JUMP_FORWARD) {
                currentBookmarkId++;
            ...
            if (currentBookmark.bookmarks.length === 0) {
                if (currentBookmark === this.activeBookmark) {
                    resolve(BookmarkFile_1.NO_MORE_BOOKMARKS);
                    return;
                }
                else {
