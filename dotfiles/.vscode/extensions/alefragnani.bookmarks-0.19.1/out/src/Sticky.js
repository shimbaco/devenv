...
class Sticky {
    static stickyBookmarks(event, activeEditorCountLine, activeBookmark, bookmarks, activeEditor) {
        let diffLine;
        ...
        if (this.hadOnlyOneValidContentChange(event)) {
            ...
            if (activeEditor.selections.length === 1) {
                if (event.contentChanges[0].text === "") {
                    updatedBookmark = this.moveStickyBookmarks("down", activeBookmark, bookmarks, activeEditor);
                }
                else if (event.contentChanges[1].text === "") {
                    updatedBookmark = this.moveStickyBookmarks("up", activeBookmark, bookmarks, activeEditor);
                }
            }
        }
        return updatedBookmark;
    }
    static moveStickyBookmarks(direction, activeBookmark, bookmarks, activeEditor) {
        let diffChange = -1;
        let updatedBookmark = false;
        let diffLine;
